...
const manageListingsCollection = async () => {
  ...
  try {
    ...
    // Check if the listings collection already exists
    const listingsCollectionExists = await db
    ...
    if (collections.length > 0) {
      ...
      // Create the listings collection
